...
```{r, file="../src/single-season-models/spOccupancy-predictions.R", include = length(psi_covs) > 0, eval = psi_cov_run}

```

```{r, include = length(psi_covs) > 0, fig.id = "figure", fig.cap = occmap_cap, eval = psi_cov_run}
...
```{r, results='asis', eval=!psi_cov_run}
cat(paste0("The best model did not include covariates for the probability of occurrence of ", spp_plural, "."))
```


```{r, include = length(marg_figs) > 0, fig.id = "figure", fig.cap = margfig_cap, results='asis', eval=psi_cov_run}
...
for (marg_fig in marg_figs) {
  ...
  margfig_cap <- paste0("Effect of ", cov_name, 
                        ...
                        PARKL, ", ", YEAR, ".")
  print(get(marg_fig))
  cat(paste0("**Figure .**", " Effect of ", cov_name, 
                        " on occurrence probability of ", spp_plural, " in ", 
                        PARKL, ", ", YEAR, "."))
  #double-printing figure captions for testing. 
  #Fig.cap in the code chunk header only prints the last caption.
  #Printing using cat() works but the text is formatted as main text and not as a figure caption
}
