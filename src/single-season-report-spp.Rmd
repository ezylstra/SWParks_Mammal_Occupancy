...
# Were camera locations included as random effects for detection or occupancy?
psi_random <- ifelse(str_detect(psi_model, "(1 | site)"), TRUE, FALSE)
p_random <- ifelse(str_detect(p_model, "(1 | site)"), TRUE, FALSE)
random_text <- ifelse(psi_random == TRUE & p_random == FALSE, "Camera locations were included as a random effect for occupancy.", ifelse(psi_random == TRUE & p_random == TRUE, "Camera locations were included as random effects for detection and occupancy.", ifelse(psi_random == FALSE & p_random == TRUE, "Camera locations were included as a random effect for detection.", "")))
...
# Calculate overall detection probability or detection probability by deployment experience level present in data
if (!("deploy_exp" %in% p_covs)) {
  ...
  deploy_text <- paste0("Overall detection probability for ", spp_plural, " was ", sprintf("%.2f", mean_det), " (95% credible interval [CI] = ", sprintf("%.2f", lcl_det),", ", sprintf("%.2f", ucl_det),").", sep="")
} else {
    deployclass_table <- deployclass_estimates(model_list$model,
                                       model_list$data,
                                       parameter = "det",
                                       lower_ci = 0.025,
                                       upper_ci = 0.975) %>%
    mutate(text = paste0(sprintf("%.2f", mean_prob), " (95% CI = ", sprintf("%.2f", ci_lower),", ", sprintf("%.2f", ci_upper), ") with ", experience_level, " personnel", sep="")) 
   
    deploy_text <- ifelse(length(p_covs)==1, paste0("Detection probability depended on the experience level of deployment personnel. Overall detection probability for ", spp_plural, " was ", paste(unlist(deployclass_table$text), collapse=", "), "."), paste0("Detection probability depended on the experience level of deployment personnel. With all other covariates in the model equal to their mean value, detection probability for ", spp_plural, " was ", paste(unlist(deployclass_table$text), collapse=", "), "."))
}
...
if ("vegclass2" %in% psi_covs) {
  ...
  lcl_occ <- NULL
  ucl_occ <- NULL
} else if ("ltaclass2" %in% psi_covs) {
  ltaclass_table <- ltaclass_estimates(model_list$model,
                                       parameter = "occ",
                                       lower_ci = 0.025,
                                       upper_ci = 0.975) %>%
    mutate(across(ci_lower:ci_upper, function(x) round(x, 2))) %>%
    ...
    select(-c(ci_lower, ci_upper)) %>%
    relocate("95% CI", .after = sd_prob)
  ltaocc_cap <- paste0("Mean occurrence estimates for ", spp_plural, 
    " in each land type association class in ", PARKL, ", ",  YEAR, ".")
  ltaocc_ft <- flextable(ltaclass_table) %>%
    align(j = 1, align = "left") %>%
    align(j = 2:4, align = "center", part = "all") %>%
    bold(bold = TRUE, part = "header") %>%
    colformat_double(j = 2:3, digits = 2) %>%
    set_caption(caption = ltaocc_cap, autonum = tab_num) %>%
    set_header_labels(vegclass = "LTA class",
                      mean_prob = "Mean",
                      sd_prob = "SD") %>%
    add_footer_lines("")
  mean_occ <- NULL
...
```{r, ft.align = "left", tab.id = "table", eval = ("ltaclass2" %in% psi_covs)}

ltaocc_ft

lta_class_text <- ifelse(PARK=="ORPI","There are four coarse landtype association classes: 1) valleys; 2) bajadas; 3) hills and 4) mountains", "hmmmm")

```

`r if("ltaclass2" %in% psi_covs){lta_class_text}`

```{r, ft.align = "left", tab.id = "table", eval = FALSE}

#eval = ("burn_severity_2011" %in% psi_covs)
burnocc_ft

```

```{r, results = "asis", eval = (!"vegclass2" %in% psi_covs)}
...
occmap_cap <- paste0("Predicted probability of ", spp_name, " occurrence in ", 
                     PARKL, " in ", YEAR, ". Dashed grey/black lines are park trails, solid grey/black lines indicate roads inside and within one kilometer of the park.")
occmap_base <- ggplot() +
