...
# Doesn't look like siteRE for occurrence or yearRE for detection are doing much
# (estimated variance small)

model_stats_orig <- model_stats
occ_specs <- c("~ slope_z + I(slope_z^2) + years_z + (1 | site)",
               "~ slope_z + I(slope_z^2) + wash_z + vegclass2 + vegclass3 + years_z + (1 | site)",
               "~ slope_z + I(slope_z^2) + roads_z + years_z + (1 | site)",
               "~ slope_z + I(slope_z^2) + wash_z + vegclass2 + vegclass3 + roads_z + years_z + (1 | site)")
det_specs <- "~ effort_z"
...
best_index <- 2 # Slope + Wash + Veg model
...
plotname <- paste0("C:/Users/erin/Desktop/Mammals/",
                   PARK, "-", SPECIES, "-OccProbMN-SlopeWashVeg-",
