...
#------------------------------------------------------------------------------#
# Specify and run first set of candidate models
#------------------------------------------------------------------------------#

# Keep a full model for detection (day2 + deploy_exp + effort)
# Try each annual covariate + a site random effect

# We'll indicate which covariates to include using the short_name column in the 
...
# All model will have the same detection model (a "full" model)
...
# Traffic model is considerably better than other models (but note that
# occupancy increased with increases in traffic!)
# None of the climate models did well (or had close to significant coefs)
# Years model is next to last (and coefficient is ~0)
BEST_ANNUAL <- "traffic"

# For detection covariates: effort is the only significant covariate

#------------------------------------------------------------------------------#
# Specify and run second set of candidate models
#------------------------------------------------------------------------------#
...
# All model will have the same detection model
...
# No. Boundary isn't close to significant.

scov_new <- list(c("aspect", "veg", "wash"))
...
# Run final model using spOccupancy package
source("src/multi-season-models/spOccupancy-MS-run-candidate-models.R")
model_stats
...
# Can view these plots, calling them by name. Available plots listed here:
  ...
  #                          PARK, "-", SPECIES, "-Occ-traffic.jpg"),
  #        plot = marginal_psi_traffic,
  #        device = "jpeg",
  #        width = 4,
  #        height = 4,
  #        units = "in",
