...
BEST_ANNUAL <- NA
  # Model with years had lower WAIC, but year effect wasn't close to significant
  # and several parameters had high Rhat values and low ESS. I think it makes
  # more sense to stick with a null model with annual REs.
...
# To use a model with a subset of those covariates, like day2 and effort:
  DET_MODELS <- list(c("day", "effort", "camera"))
...
# power from the model for occurrence.
  ...
  # Selecting the model that includes boundary, even though WAIC is a little 
  # lower because convergence measures are better.
