...
# Model with 10-month precipitation (ppt10) is better than the rest
# Trend model is ok, not great (coefficient is negative, but not significant)
BEST_ANNUAL <- "ppt10"

# For detection covariates: nothing is close to significant, so we'll use null
# model.
...
# Yes(ish). Vegclass is borderline, but we'll keep it in for now.
