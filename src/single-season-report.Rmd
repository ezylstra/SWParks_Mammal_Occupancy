...
# any new detections
if(PARK != "ORPI")detects_new <- detects %>%
  mutate(visit = ifelse(yr == YEAR, "Current", ifelse(yr > YEAR, "Future", "Past"))) %>%
  filter(visit != "Future") %>%
  ungroup() %>%
...
new_species_text = ifelse(PARK == "ORPI", "Past observation and detection data are not available for comparison.", ifelse(nrow(obs_new) > 0 & nrow(detects_new) == 0, paste("In ",YEAR, ", our cameras captured ",paste(unlist(detects_new$name), collapse=", "), " for the first time during SODN monitoring.", sep=""), ifelse(nrow(detects_new) > 0, paste("In ",YEAR, ", we detected ",paste(unlist(detects_new$name), collapse=", "), " for the first time during a sampling occasion. ", sep=""), "All species listed in Table X were observed and detected during previous SODN monitoring.")))
