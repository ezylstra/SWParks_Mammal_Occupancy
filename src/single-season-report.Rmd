...
For each species, we first evaluated whether day (using linear and quadratic terms), 
...
with little to no explanatory power (proportion of posterior samples that were < 0 if the posterior 
mean was < 0 or the proportion of posterior samples > 0 if the posterior mean was > 0 [f-value] < 0.9) ). 
...
obs_new_run = ifelse(nrow(obs_new) > 0, TRUE, FALSE)
...
new_species_run = ifelse(nrow(obs_new) > 0 | nrow(detects_new) > 0, TRUE, FALSE)

new_species_text = ifelse(nrow(obs_new) > 0 & nrow(detects_new) == 0, paste("In ",YEAR, ", our cameras captured ",paste(unlist(detects_new$name), collapse=", "), " for the first time during SODN monitoring.", sep=""), ifelse(nrow(detects_new) > 0, paste("In ",YEAR, ", we detected ",paste(unlist(detects_new$name), collapse=", "), " for the first time during a sampling occasion. ", sep=""), "uh oh"))
...
species_text_obs <- ifelse(genera_count_obs > 0, paste("We captured a total of ", species_count_obs, " mammal species plus ", genera_count_obs, " mammalian families or genera", sep=""), paste("We captured a total of ", species_count_obs, " mammal species", sep=""))
...
species_text_det <- ifelse(species_count_obs == species_count_det & genera_count_obs == genera_count_det & genera_count_obs == 0, paste("had at least one detection of each of the ", species_count_obs, " species", sep=""), ifelse(species_count_obs == species_count_det & genera_count_obs == genera_count_det & genera_count_obs > 0, paste("had at least one detection of each of the ", species_count_obs, " species and ", genera_count_obs, " genera/families", sep=""), ifelse(species_count_obs > species_count_det & genera_count_obs == 0, paste("only had detections for ", species_count_det, " of the ", species_count_obs, " species", sep=""), ifelse(species_count_obs > species_count_det & genera_count_obs == species_count_det, paste("only had detections for ", species_count_det, " of the ", species_count_obs, " species but all ",genera_count_obs, " of the genera/families", sep=""), ifelse(species_count_obs == species_count_det & genera_count_obs > genera_count_det, paste("had at least one detection of each of the ", species_count_obs, " species but only had detections for ", genera_count_det, " of the ", genera_count_obs, " genera/families", sep=""),"hmmm")))))

```

`r species_text_obs` in `r total_photos` photos from the `r nrow(events)` cameras during the `r YEAR` camera deployment. During the `r nrow(occasions)` sampling occasions (weeks) in `r YEAR`, we captured `r nrow(dat)` photos and `r species_text_det` (Table #). 
`r if(new_species_run){new_species_text}`
