...
We detected a total of `r species_count` mammal species plus `r genera_count` mammalian 
families or genera on the `r nrow(events)` 
cameras during the `r nrow(occasions)` sampling occasions in `r YEAR` (Table #). 
`r if(detects_new_run){detects_new_text}`Overall, we recorded a 
total of `r total_photos` photos of mammals, `r nrow(dat)` of which occurred within 
the `r nrow(occasions)` sampling occasions. The number of photos containing a mammal during
the annual monitoring at `r PARKL` has ranged from `r min(dat_park$n)` to `r max(dat_park$n)`.
It is hard to know exactly why this range has varied throughout the years, but we are hopeful 
to gain insight into this question in the future via multi-season occupancy modeling.

**Take a look and see if there are any interesting or notable detections, even if there weren’t many of them.**
