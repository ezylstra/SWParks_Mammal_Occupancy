...
SPECIES <- c("UNLE", "LECA", "LEAL") # all jackrabbits
...
DET_MODELS <- list(c("deploy_exp", "day2")) 
...
# the model (code below is not elegant, but it works)
  ...
  # above, but specify to be sure)
   OCC_MODELS <- list(c("boundary", "aspect", "wash", "veg"), c("boundary", "wash", "veg"), 
                      c("boundary", "aspect", "veg"), c("boundary", "wash"))
   DET_MODELS <- list(c("deploy_exp", "day"))
