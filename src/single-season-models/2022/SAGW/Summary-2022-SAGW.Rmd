...
if ("deploy_exp" %in% p_covs) {
  alpha_samples <- model_list$model$alpha.samples[ ,c("(Intercept)", "deploy_exp")]
  X0 <- as.vector(c(1, 2))
  probs_logit <- alpha_samples %*% X0
} else {
  probs_logit <- model_list$model$alpha.samples[ ,"(Intercept)"]
}
...
if ("effort" %in% p_covs) {
  if (estimates$Mean[estimates$Covariate == "effort_z"] > 0) {
    effortpos <- TRUE
  } else {
    effortneg <- TRUE
  }
