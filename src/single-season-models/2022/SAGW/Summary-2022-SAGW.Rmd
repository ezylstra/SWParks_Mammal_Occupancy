...
**Table 1.** Start and end dates for each sampling occasion in `r PARKL`, 
`r YEAR`.
...
detections is the number of detections to be used in an occupancy modeling 
framework (maximum of one photographic detection per camera location per 
sampling period). No. locations is the unique number of camera locations where 
the species was photographed.
...
**Table X.** Parameter estimates from a model for `r spp_plural` in `r PARKL`, 
`r YEAR`. SD = Standard deviation; 95% CI = 95% credible interval. Rhat
...
logit scale and all continuous covariates were standardized by their 
respective means and standard deviations prior to analysis.
...
Text about overall estimate(s).

Probably don't need figures, just a description of covariate effects?
(maybe if day2 is significant, can provide day when detection probabilities
are at a maximum or minimum)

## Estimated occurrence probabilities

Text about overall estimate(s) [by vegclass if relevant]
...
```{r CALA-test-map, fig.width = 5, fig.height = 3.5, dpi = 300, echo = FALSE}
...
# But maybe keep as is, since this is just a reference doc to find 1-2 figs
# they'll want to include in final reports.
