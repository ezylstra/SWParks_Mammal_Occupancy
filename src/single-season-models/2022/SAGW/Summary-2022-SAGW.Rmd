...
```{r deployment-plot, fig.width = 5, fig.height = 3.5, echo = FALSE}
segs
```
...
**Table 2.** Mammal species detected in `r PARKL`, `r YEAR`. No. photos is the 
the total number of photographs obtained (multiple photos may occur at the same 
camera location in the same day). No. detections is the number of detections to 
be used in an occupancy modeling framework (maximum of one photographic
detection per camera location per sampling period). No. locations is the unique 
number of camera locations where the species was photographed in `r YEAR`.
