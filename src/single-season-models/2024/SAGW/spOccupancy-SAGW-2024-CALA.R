...
# Trying additional models after looking at full model
...
# only effort significant (f~0.985) but day (0.840) and day2 (0.818) both f>0.8, so I am going to try models with just those covariates and effort as well 
# day and day2 still not f>0.9 in new models
...
# elev is the only significant covariate for top 3 models (f~1.00)
...
# power from the model for occurrence.
  ...
  (f <- apply(samps, 2, function(x) ifelse(mean(x) > 0, sum(x > 0) / length(x),
   ...
   # elev (1.000) and effort (0.985) both significant; Rhat(1.02), ESS (1239), ppc.sites (0.197), ppc.reps (0.727) all look good
...
# both elev and effort exclude zero
