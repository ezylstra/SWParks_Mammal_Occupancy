...
# Adding new models after looking at the full model
...
# no significant covariates for full model, day is closest with f~0.855; going to try adding a null model and a day only model

# Pick which detection covariates to use in subsequent candidate models
# (for now, can use covariates with f >= 0.9)
# The null det model is the best
...
# trail was sig f>0.9 in all of top 3 models, and trail + north was sig in 3rd best model
# Only model 9 (2nd best WAIC) has a ppc.reps>0.1, other two are 0.091 and 0.093 respectively
# Going to try models with trail and north and just trail
...
# north + trail model is better (lower WAIC) has good Rhat, ESS, ppc.sites, but ppc.reps is 0.099 and only north f (0.98) is f>0.9, trail f is 0.89; just trail also has good Rhat, ESS, ppc.sites, and ppc.reps>0.1 (0.111), trail f is>0.9 (0.942)
# The trail only model has an acceptable ppc.reps, but it is nonetheless low...
  
# I'm going to add a north only model now and compare north, trail, and trail + north
  # The north only model is the best, Rhat, ESS, ppc.reps, and ppc.sites are all acceptable; north f=0.98
