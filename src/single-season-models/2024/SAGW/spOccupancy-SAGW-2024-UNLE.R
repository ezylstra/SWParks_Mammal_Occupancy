...
# PPC sites value for the full model is quite low (0.037) but everything else looks good 
...
# for SAGW need to key out elev boundary and elev roadbound combos

# # visualize veg vs. elev and slope
# spatial_covs %>%
...
OCC_MODELS <- list(c("aspect", "veg", "wash", "burn", "roads"),
                   ...
                   c("aspect", "veg", "wash", "burn", "roadbound"), # my additions to Cheryl's new list of spatial covariate combos start here
...
# I checked the top two models; both have good Rhat, ESS, ppc.reps, but have issues with ppc.sites
# Model 10 (best model) has f>0.9 for slope (1.000), wash (0.947), deploy_exp (0.920), and effort (0.903)
...
# power from the model for occurrence.
  ...
  OCC_NULL <- FALSE
  OCC_MODELS <- list(c("slope", "wash"))

  # Change detection part of model (if needed)
  DET_NULL <- TRUE
  DET_MODELS <- list(c("deploy_exp"),
                       c("deploy_exp", "effort"))
  # rm(DET_MODELS)
  ...
  # When I ran the model with slope, wash, deploy_exp, and effort, effort was no longer significant (f~0.898)
  # I am going to try adding another model without effort --> now deploy_exp is no longer significant (f~0.895)
  # I am adding a null model for detection; the null model is the best model and wash and slope remain f>0.9; null model has a lower ppc.sites than the 2 models with detection covariates, but all three of them are ppc.sites<0.1
...
# Spatial variation is not adequately described but temporal variation is 
...
# best model has random effects on both detection and occurence; slope and wash stay f>0.9 
