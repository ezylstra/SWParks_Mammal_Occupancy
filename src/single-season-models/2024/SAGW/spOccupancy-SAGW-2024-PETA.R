...
DET_MODELS <- list(c("burn", "day2", "deploy_exp", "effort"), c("day", "effort"))
DET_MODELS <- list(c("day", "effort")) # testing a model with just effort and day since effort f=0.868
...
(f <- apply(samps, 2, function(x) ifelse(mean(x) > 0, sum(x > 0) / length(x),
                                         sum(x < 0) / length(x))))
# only day f>0.9 (f = 0.912)
# even the model with just effort and day, effort f<0.9
...
(f <- apply(samps, 2, function(x) ifelse(mean(x) > 0, sum(x > 0) / length(x),
                                         sum(x < 0) / length(x))))
# "Best" (lowest WAIC) model is #7 and has boundary (f=0.799), wash (f=0.938), veg2 (f=0.635) & veg3 (f=0.931)
# 2nd best #4 roads, wash, veg, roads f<0.9
# 3rd best #14 has lower ESS, but still low Rhat and acceptable ppc.sites and ppc.reps, slope and wash f>0.9
# even tho slope f>0.9 in model #14, veg is in more of the top models than slope
# going to check models with slope and wash or veg and wash in next step
...
# power from the model for occurrence.
  ...
  (f <- apply(samps, 2, function(x) ifelse(mean(x) > 0, sum(x > 0) / length(x),
                                           sum(x < 0) / length(x))))
# model with wash + veg has a low rhat (1.01) min.ESS of 1500 (compared to 675 for slope + wash), acceptable ppc.sites/reps and a lower waic 
# "best" model of wash + veg + day all covariates f>0.9
...
estimates
# none of the covariates exclude0
