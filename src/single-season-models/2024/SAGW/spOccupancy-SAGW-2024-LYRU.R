...
# deploy_exp (0.925) and effort (0.961) had f>0.9; day2 f>0.8 (0.833)
...
# Best model based on WAIC (15) only veg class, deploy_exp, and effort sig (this model has a higher ESS than 2nd and 3rd best)
# 2nd best model (11) Rhat is 1.14 which is decently higher than 1st and 3rd best
# 3rd best model (4) only veg class, deploy_exp, and effort sig
# Going to build a model with only veg class as my occ covariate
...
# power from the model for occurrence.
  ...
  (f <- apply(samps, 2, function(x) ifelse(mean(x) > 0, sum(x > 0) / length(x),
                                           sum(x < 0) / length(x))))
# veg class + deploy_exp + effort model still has all covariates f>0.9, Rhat is good, ESS is a little low (413), ppc.sites and ppc.reps are good
