...
DET_NULL <- TRUE # effort only model had an f value of ~0.87, so also going to add a null model

# Create a full model for detection
#DET_MODELS <- list(c("burn", "day2", "deploy_exp", "effort"))
DET_MODELS <- list(c("burn", "day2", "deploy_exp", "effort"), c("effort")) # also trying a model with just effort since for the full model, effort f was ~ 0.8
...
# Look at results
best_index <- 1 # change this to the model number if you are comparing multiple models
...
rm(DET_MODELS) # the null model for detection was best
...
cor_df %>%
  arrange(desc(corr)) %>%
  dplyr::filter(abs(corr) >= 0.7) # need to comment out elev/roadbound, boundary/elev
...
# The best model based on WAIC had east, north, roads, wash, vegclass2, and vegclass3 -- all f values were >0.9
# F values for veg class were slightly lower (vc2 ~0.67 and vc3~0.90, so I am going to try running this best model with and without the veg covariate)
  
# Change occupancy part of model (if needed)
  OCC_NULL <- FALSE
  OCC_MODELS <- list(c("aspect", "roads", "wash", "veg"),
                     c("aspect", "roads", "wash")) # the model without veg ended up having a lower WAIC, an Rhat of 1.01, and sufficiently described spatial and temporal variation
...
estimates # none of the covariates exclude0 
