...
SPECIES <- c("UNLE","LECA","LEAL")  # all jackrabbits lumped together
...
# power from the model for occurrence.
  ...
  # OCC_NULL <- FALSE
  # OCC_MODELS <- list(c("slope"), c("boundary", "slope"), c("slope", "trail"))
...
# the model (code below is not elegant, but it works)
  ...
  # above, but specify to be sure)
   OCC_MODELS <- list(c("slope", "roads"), "roads", "slope")
