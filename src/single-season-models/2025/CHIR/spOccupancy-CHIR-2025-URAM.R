...
# best model: boundary (0.913), deploy_exp (0.954) f>0.9; east borderline (0.897)
# 2nd best model none of the occ covariates f>0.9 (roadbound semi-close, 0.858); deploy_exp (0.919) f > 0.9 and effort borderline (0.899)
# 3rd best model similarly has none of occ cov f >0.9 with deploy_exp f>0.9 and effort borderline

# going to move forward with adding a null occ model, boundary + east, just boundary, deploy_exp + effort, and just deploy_exp
...
# power from the model for occurrence.
  ...
  OCC_NULL <- TRUE
  OCC_MODELS <- list(c("boundary", "east"),
                     c("boundary"))
  ...
  # Best model (#1) = ~1 (null for occ) ~ deploy_exp (0.930) and effort (0.925), pcc.sites, ppc.reps, ESS, Rhat all good
