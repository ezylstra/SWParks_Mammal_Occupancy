...
# deploy_exp (0.900) and effort (0.939) f>0.9; since deploy_exp is borderline, going to also include a candidate model with only effort
...
# lta is in many of the top 5 models, but never f > 0.9 (not even very close)
# slope often close to 0.9
# effort usually just below 0.9, deploy_exp not close; going to try with effort and a null for det next time
# 10/07/2025 going to opt for slope (0.883) + trailpoi (1.000) + effort (0.865) since WAIC is lowest and none of these models end up turning out that nice
...
# power from the model for occurrence.
  ...
  #OCC_NULL <- FALSE
  #OCC_MODELS <- list(c("slope", "trailpoi"),
                      # c("trailpoi"),
                      # c("elev", "roads"),
                      # c("elev", "roadbound"))
 ...
 # STAT <- "model_no"
  ...
  #                                           sum(x < 0) / length(x))))
   ...
   # Best model (#5) now ~slope (0.883) + trailpoi (1.000) ~effort (0.865) # effort is not quite close enough to 0.9 that I want to include it in the model
   # 2nd best (#1) ~slope (0.889) + trailpoi (1.000) ~1 (going to opt for this one given how close slope is to 0.9)
   # 3rd/4th best (tied = #7 and #8) ~elev (0.976) + roads or roadbound (0.999) ~effort (0.898)
