...
# only effort (0.961) f > 0.9, day is fairly close though (0.879)
# ppc.sites is low
...
# top 6 models all have slope, none of them have acceptable ppc.sites
# best model (#14) ~slope (1.000) + pois (0.997) ~effort (0.946)
# 2nd and 3rd best models also have slope ~1.000 and all cov f > 0.9
...
# need to add RE to address low ppc.sites
...
# the model (code below is not elegant, but it works)
 ...
 STAT <- "waic"
  ...
  (f <- apply(samps, 2, function(x) ifelse(mean(x) > 0, sum(x > 0) / length(x),
                                        sum(x < 0) / length(x))))
# Best model is ~pois + slope + RE ~effort + RE BUT effort f no longer >0.9 (0.871)
  # going to add in a null model for det 
   # New best model is ~pois + slope + RE ~1 + RE; all cov f>0.9, Rhat, ESS, ppc.sites, and ppc.reps acceptable 
