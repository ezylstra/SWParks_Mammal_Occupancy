...
(f <- apply(samps, 2, function(x) ifelse(mean(x) > 0, sum(x > 0) / length(x),
                                         sum(x < 0) / length(x))))
# Model 7 is best, has acceptable Rhat, ppc.sites, ESS, and ppc.reps
# boundary (f~0.997), day2 (~0.943), effort (f~0.913) (veg and wash f<0.9)
...
# with veg and wash removed the model still has f>0.9 for boundary, day2, and effort 
