...
# day2 and deploy_exp f>0.9 (~0.999 and 0.957) 
...
# The top 3 models all have acceptable values for Rhat, ESS, and ppc.reps, but they all also have low ppc.sites values (~0.05)
...
# Top 3 models all slope + other (trail, trailpoi, bound) + wash + deploy_exp + day2 (all covariates f>0.9 except wash)
# Going to try out top 3 model slope + other combos (removing wash)
  OCC_NULL <- FALSE
 ...
 # Look at model output and f values
  ...
  # The slope + trail + day + day2 model has all covariates (f>0.9); Rhat = 1.01, ESS = 1376, ppc.sites is low (0.079), and ppc.reps is good (0.571)
...
# DID NOT ADEQUATELY DESCRIBE SPATIAL VARIATION IN DETECTION 

# CIRCLING BACK: trying RE again with doubled burn in and sample size since Rhat was high for "best" model and ESS was low for first and second best models
# Set MCMC parameters
N_SAMPLES <- 16000
N_BURN <- 8000
N_THIN <- 8
N_CHAINS <- 3
  ...
  # After increasing the sample size/burn in:
  # BEST = model 2, Rhat = 1.03, ESS = 444, ppc.sites = 0.421, ppc.reps = 0.513
  STAT <- "model_no"
  if (STAT == "model_no") {
   # If STAT == "model_no", specify model of interest by model number in table
    best_index <- 4
 ...
 # Look at model output
  ...
  # after doubling sample size and burn in, deploy_exp f no longer >0.9
  # running again with a 2nd detection candidate model (day2 vs day2 + deploy_exp) 
