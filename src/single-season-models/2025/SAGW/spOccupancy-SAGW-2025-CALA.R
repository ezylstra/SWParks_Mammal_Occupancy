...
# BEST model (#10): slope (f~1.000) + trail (0.981) + wash (0.889) + effort (0.908)
# 2nd best (#4): road (0.938) + veg2 (1.000) + veg3 (1.000) = effort (0.900)
# Both best and second best models have issue with ppc.sites but good for Rhat, ppc.reps, and ESS
# Going to try building a road + trail + effort model and a road + veg + effort model
...
# power from the model for occurrence.
 ...
 # Look at model output and f values
  ...
  # New best model = roads + veg2 + veg3 + effort; all cov f>0.9, Rhat, ESS, and ppc.reps are good, but ppc.sites is low 
# View trace plots
...
# the model (code below is not elegant, but it works)
  ...
  # Best model = roads + veg + effort + RE (Rhat, ppc.reps, ppc.sites, ESS all good)
